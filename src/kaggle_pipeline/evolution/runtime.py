...
checkpointing, ensembling (when enabled) and finalization, and answers the
questions the orchestrator asks: can a batch/model still start, should training
stop, should we checkpoint, should ensembling begin.
...
class RuntimeManager:
    ...
    def __post_init__(self) -> None:
        ...
        self.training_deadline = self.start_time + self.max_runtime_seconds - reserves
    ...
    def remaining_training_seconds(self) -> float:
        return max(0.0, self.training_deadline - time.monotonic())
